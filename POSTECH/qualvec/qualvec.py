...
tf.app.flags.DEFINE_integer("size", 900, "Size of each model layer.")
...
def create_model(session, state):
    ...
    model = qualvec_model.QualVecModel(
...
def qualvec():
  with tf.Session() as sess:
    ...
    for set_name in ['train', 'dev', 'test']:
        source_file = os.path.join(FLAGS.data_dir, "{0}.src".format(set_name))
        target_file = os.path.join(FLAGS.data_dir, "{0}.mt".format(set_name))
    ...
    with tf.Session() as sess:
        ...
        while sentences:
            ...
            encoder_inputs, decoder_inputs, target_weights = model.get_decode_batch(
                {bucket_id: [(source_ids, target_ids)]}, bucket_id)
            # Get output logits for the sentence.
            w_t, _, outputs  = model.step(sess, encoder_inputs, decoder_inputs,
